...
import reapermod.util.GeneralUtils;
import reapermod.util.KeywordInfo;
import reapermod.util.Sounds;
import reapermod.util.TextureLoader;
...
@SpireInitializer
public class TheReaperMod implements
        EditCardsSubscriber,
    ...
    public void receiveEditCharacters() {
        TheReaper.Meta.registerCharacter();
    }
    ...
    private static String checkResourcesPath() {
        String name = TheReaperMod.class.getName(); //getPackage can be iffy with patching, so class name is used instead.
        ...
        if (!resources.exists()) {
            throw new RuntimeException("\n\tFailed to find resources folder; expected it to be at  \"resources/" + name + "\"." +
                    ...
                    "\t\"private static final String resourcesFolder = checkResourcesPath();\"\n" +
                    "\tat the top of the " + TheReaperMod.class.getSimpleName() + " java file.");
        }
    ...
    private static void loadModInfo() {
        Optional<ModInfo> infos = Arrays.stream(Loader.MODINFOS).filter((modInfo)->{
            ...
            return initializers.contains(TheReaperMod.class.getName());
        }).findFirst();
        if (infos.isPresent()) {
            info = infos.get();
            modID = info.ID;
