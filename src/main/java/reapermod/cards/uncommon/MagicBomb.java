...
public class MagicBomb extends BaseCard {
    public static final String ID = makeID("MagicBomb"); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
...
//        addToBot(new DamageAction(m, new DamageInfo(p, damage, DamageInfo.DamageType.NORMAL), AbstractGameAction.AttackEffect.LIGHTNING));
        addToBot(new DamageAllEnemiesAction(p, damage + (p.drawPile.size() * magicNumber), DamageInfo.DamageType.NORMAL, AbstractGameAction.AttackEffect.LIGHTNING));
    }
    @Override
